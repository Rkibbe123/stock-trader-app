from trading_script import set_data_dir, load_latest_portfolio_state
import os
import argparse
import pandas as pd
import json
...
def run_automated_trading(api_key: str, model: str = "gpt-4", data_dir: str = "Start Your Own", dry_run: bool = False):
    ...
    if portfolio_file.exists():
        ...
        try:
            ...
            print("Invalid input. Using default starting cash of $10,000.")
            cash = 10000.0
    ...
    print(f"- You have ${cash:,.2f} in cash available for new positions")
...
def generate_trading_prompt(portfolio_df: pd.DataFrame, cash: float, total_equity: float) -> str:
    ...
    prompt = f"""You are an aggressive professional portfolio analyst. Here is your current portfolio state as of {today}:
...
- Take larger position sizes and be willing to allocate a significant portion of available cash to high-conviction trades
- Seek multiple trade opportunities, even if it means higher portfolio turnover
- Prioritize growth and momentum stocks, and be willing to take higher risks for higher potential returns
- Do not be conservative—be bold and opportunistic in your recommendations

Analyze the current market conditions and provide specific, aggressive trading recommendations.
...
def call_openai_api(prompt: str, api_key: str, model: str = "gpt-4") -> str:
    """Call OpenAI API and return response (works for OpenAI or Azure OpenAI if endpoint is set)"""
    try:
        import openai
    except ImportError:
...
def main():
    ...
    parser.add_argument(
        "--model",
        default="rk-chatgpt-stockpicker",
        help="Model name (OpenAI) or deployment name (Azure OpenAI). Default: rk-chatgpt-stockpicker"
    )
